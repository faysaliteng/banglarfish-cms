...
import { Search, ShoppingBag, User, Menu, Phone, LogOut, LayoutDashboard } from "lucide-react";
...
export function Header() {
  ...
  const phone = settings?.storePhone || "+880 1000-000000";
  ...
  return (
    <header className="sticky top-0 z-40 bg-background/95 backdrop-blur border-b">
      ...
      <div className="container-x flex items-center gap-3 md:gap-6 py-4">
        ...
        <form action="/shop" className="hidden md:flex flex-1 max-w-xl mx-auto relative">
          <input
            ...
            placeholder="Search for hilsa, prawn, rohu..."
        ...
        <div className="ml-auto flex items-center gap-3">
          {!loading && !user && (
            <Link to="/auth" search={{ next: "/account" }} className="hidden sm:flex items-center gap-1.5 text-sm hover:text-primary">
              <User className="h-5 w-5" /> <span className="hidden lg:inline">Sign in</span>
            </Link>
          )}
          {!loading && user && (
      ...
      <nav className="hidden md:block border-t">
        <div className="container-x flex items-center gap-1 py-2 text-sm font-medium overflow-x-auto">
          <Link to="/" className="px-3 py-1.5 rounded-md hover:bg-muted [&.active]:text-primary" activeOptions={{ exact: true }}>Home</Link>
          <Link to="/shop" className="px-3 py-1.5 rounded-md hover:bg-muted [&.active]:text-primary">Shop All</Link>
          {categories.map((c) => (
            <Link
              key={c.slug}
              to="/category/$slug"
              params={{ slug: c.slug }}
              className="px-3 py-1.5 rounded-md hover:bg-muted whitespace-nowrap [&.active]:text-primary"
            >
              {c.name}
            </Link>
          ))}
          {showRecipes && <Link to="/recipes" className="px-3 py-1.5 rounded-md hover:bg-muted whitespace-nowrap [&.active]:text-primary">Recipes</Link>}
          {showBlog && <Link to="/blog" className="px-3 py-1.5 rounded-md hover:bg-muted [&.active]:text-primary">Blog</Link>}
          {headerLinks.map((l, i) => (
            <a key={i} href={l.href} className="px-3 py-1.5 rounded-md hover:bg-muted whitespace-nowrap">{l.label}</a>
          ))}
        </div>
      </nav>
      {open && (
        <div className="md:hidden border-t bg-background">
          <div className="container-x py-3 flex flex-col gap-1 text-sm">
            ...
            {user ? (
              <Link to="/account" onClick={() => setOpen(false)} className="py-2 font-medium">My account</Link>
            ) : (
              <Link to="/auth" search={{ next: "/account" }} onClick={() => setOpen(false)} className="py-2 font-medium">Sign in</Link>
            )}
            {isAdmin && (
              <Link to="/admin" onClick={() => setOpen(false)} className="py-2 font-medium">Admin panel</Link>
            )}
            ...
            {showRecipes && <Link to="/recipes" onClick={() => setOpen(false)} className="py-2">Recipes</Link>}
            {showBlog && <Link to="/blog" onClick={() => setOpen(false)} className="py-2">Blog</Link>}
          </div>
        </div>
      )}
    </header>
