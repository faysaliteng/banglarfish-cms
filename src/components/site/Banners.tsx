      ...
      <section className={`container-x ${className}`}>
        {items.map((b) => (
          <Wrap key={b.id} href={b.href} className="group relative block rounded-2xl overflow-hidden mb-4 last:mb-0">
            {b.image && <img src={b.image} alt={b.title} loading="lazy" className="w-full h-[220px] md:h-[320px] object-cover group-hover:scale-[1.02] transition-transform duration-500" />}
            {(b.title || b.subtitle) && (
    ...
    <section className={`container-x grid md:grid-cols-2 gap-5 ${className}`}>
      {items.map((b) => (
        <Wrap key={b.id} href={b.href} className="group relative block rounded-2xl overflow-hidden min-h-[180px] bg-muted">
          {b.image && <img src={b.image} alt={b.title} loading="lazy" className="absolute inset-0 h-full w-full object-cover group-hover:scale-105 transition-transform duration-500" />}
