...
export type MediaItem = { id: string; name: string; url: string; size: string; uploadedAt: string };
...
export const adminListPages = createServerFn({ method: "GET" }).handler(async (): Promise<CmsPage[]> => {
  ...
  return rows.map((p) => ({
    ...
    noindex: p.noindex, template: p.template, showInHeader: p.showInHeader, showInFooter: p.showInFooter, sort: p.sort,
  }));
});
...
export const adminListMedia = createServerFn({ method: "GET" }).handler(async (): Promise<MediaItem[]> => {
  ...
  return rows.map((m) => ({ id: m.id, name: m.name, url: m.url, size: m.size, uploadedAt: m.uploadedAt.toISOString().slice(0, 10) }));
});
...
export const adminUpsertBlog = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }): Promise<{ id: string }> => {
    ...
    const values = {
      ...
      status: data.status, publishedAt: data.status === "published" ? new Date() : null,
    };
    let id: string;
    if (data.id) {
